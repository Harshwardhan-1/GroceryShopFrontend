...
export default function OrderPage(){
    ...
    const ChooseQuantity=async(id)=>{
        ...
        const response=await axios.post("https://grocery-store-backend-2.onrender.com/api/product/getquantity",send,{withCredentials:true});
    ...
    const handleAddToCart=async()=>{
     ...
     const response=await axios.post("https://grocery-store-backend-2.onrender.com/api/cart/cartItems",addtocart,{withCredentials:true});
...
const handlePayment = async () => {
  ...
  try {
    ...
    const { data } = await axios.post(
      "https://grocery-store-backend-2.onrender.com/api/cart/AddPayment",
      sendData,
...
//handle payment button end
  ...
  <img src={`https://grocery-store-backend-2.onrender.com/uploads/${harsh.image}`} alt="" />
