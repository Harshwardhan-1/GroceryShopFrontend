...
export default function AddToCart(){
    ...
    useEffect(()=>{
        const fetch=async()=>{
            const response=await axios.get("https://grocery-store-backend-3.onrender.com/api/cart/cartItems",{withCredentials:true});
    ...
    const handleDelete=async(id)=>{
        const send={id};
        const response=await axios.post("https://grocery-store-backend-3.onrender.com/api/cart/cartDelete",send,{withCredentials:true});
    ...
    return(
        <>
          ...
          <img src={`https://grocery-store-backend-3.onrender.com/uploads/${st.image}`} alt={st.name} className="cart-image"/>
