...
export default function ProductsPage(){
    ...
    useEffect(()=>{
        const fetch=async()=>{
            const response=await axios.get("https://groceryshopbackend-3.onrender.com/api/product/allimages",{withCredentials:true});
     ...
     const handle=async(id)=>{
        const send={id};
        const response=await axios.post("https://groceryshopbackend-3.onrender.com/api/product/particular",send,{withCredentials:true});
  ...
  <div className="products-wrapper">
    {
      data && data.map((st, index) => (
        <div className="product-card" key={index}>
          ...
          <img src={`https://groceryshopbackend-3.onrender.com/uploads/${st.image}`} alt={st.name} className="product-image" />
